...
const MAX_INSTANCES =
  Number.isFinite(maxInstancesEnv) && maxInstancesEnv > 0
    ? maxInstancesEnv
    : 10;
let cachedApp = null;
...
async function getApp() {
  ...
  appInitializationPromise = (async () => {
    try {
      ...
      const { createServer } = await Promise.resolve().then(() =>
        __importStar(require("../backend-dist/serverless")),
      );
    ...
    .https.onRequest(async (req, res) => {
      ...
      const setCorsHeaders = () => {
        ...
        res.setHeader(
          ...
          "Content-Type, Authorization, Accept, X-Requested-With, Origin, Access-Control-Request-Method, Access-Control-Request-Headers, X-Tenant-Slug",
        );
      ...
      if (req.method === "OPTIONS") {
        console.log(
          "[Functions] Handling OPTIONS preflight request from origin:",
          origin,
        );

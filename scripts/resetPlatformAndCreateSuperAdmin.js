...
async function clearPlatformData(db) {
  ...
  if (!platformTenants.empty) {
    ...
    const platformUsers = await db.collection('users')
  ...
  if (allPlatformAdmins.size > 0) {
    console.log(`Found ${allPlatformAdmins.size} additional platform admin users to delete`);
...
async function main() {
  ...
  try {
    ...
    const confirm = await question('⚠️  This will DELETE existing platform data. Continue? (yes/no): ');

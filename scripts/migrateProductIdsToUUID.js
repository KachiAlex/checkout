...
async function ensureInitialized() {
  if (admin.apps.length === 0) {
    ...
    if (emulatorHost) {
      ...
      console.log(`Using Firestore emulator at ${emulatorHost} with project ${projId}`);
...
async function migrateProductsForTenant(tenantId, db) {
  ...
  const productsSnapshot = await productsRef.where('tenantId', '==', tenantId).get();
    ...
    try {
      ...
      ordersSnapshot.forEach((orderDoc) => {
        ...
        if (orderData.items && Array.isArray(orderData.items)) {
          ...
          if (needsUpdate) {
            orderUpdates.push(
              orderDoc.ref.update({ items: updatedItems })
            );
          }
        }
      });
      ...
      poSnapshot.forEach((poDoc) => {
        ...
        if (poData.items && Array.isArray(poData.items)) {
          ...
          if (needsUpdate) {
            poUpdates.push(
              poDoc.ref.update({ items: updatedItems })
            );
          }
        }
      });
      ...
      if (poUpdates.length > 0) {
        ...
        console.log(`      ✅ Updated ${poUpdates.length} purchase order records`);
      }
      ...
      grnSnapshot.forEach((grnDoc) => {
        ...
        if (grnData.items && Array.isArray(grnData.items)) {
          ...
          if (needsUpdate) {
            grnUpdates.push(
              grnDoc.ref.update({ items: updatedItems })
            );
          }
        }
      });
      ...
      returnsSnapshot.forEach((returnDoc) => {
        ...
        if (returnData.items && Array.isArray(returnData.items)) {
          ...
          if (needsUpdate) {
            returnUpdates.push(
              returnDoc.ref.update({ items: updatedItems })
            );
          }
        }
      });

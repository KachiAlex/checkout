...
function getProjectId() {
  ...
  try {
    ...
    if (fs.existsSync(firebasercPath)) {
      ...
      return defaultProject || checkoutProject || Object.values(firebaserc.projects || {})[0];
    }
  } catch (error) {
...
async function ensureInitialized() {
  if (admin.apps.length === 0) {
    ...
    if (emulatorHost) {
      ...
      console.log(`Using Firestore emulator at ${emulatorHost} with project ${projectId}`);
      ...
      try {
        ...
        console.error('1. Set environment variables: FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, FIREBASE_PRIVATE_KEY');
...
async function removeDuplicates(db) {
  ...
  for (const { records, key } of duplicates) {
    ...
    console.log(`Group ${key}: Keeping ${keep.id}, removing ${toRemove.length} duplicates`);
  ...
  try {
    switch (command) {
      case 'duplicates':
        ...
        if (duplicates.length === 0) {
          ...
          duplicates.forEach(({ key, records }) => {
            ...
            records.forEach((r, i) => {
              console.log(`    ${i === 0 ? '[KEEP]' : '[REMOVE]'} ${r.id} - qty: ${r.quantity}, created: ${r.createdAt?.toDate?.() || r.createdAt || 'unknown'}`);
            });
          });
        }
        break;
      ...
      case 'remove-duplicates':
        ...
        console.log(`Removed ${result.removed} duplicate records, kept ${result.kept} records.`);
      ...
      default:
        ...
        console.log('  duplicates         - Find duplicate inventory entries (default)');
        console.log('  remove-duplicates  - Remove duplicate inventory entries');

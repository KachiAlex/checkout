...
contextBridge.exposeInMainWorld('posApp', {
  ...
  listNativeDevices: async (): Promise<NativeDeviceSummary[]> => ipcRenderer.invoke('native-devices:list'),
  scanBluetoothDevices: async (timeoutMs?: number): Promise<NativeDeviceSummary[]> =>
    ipcRenderer.invoke('native-devices:scan', timeoutMs),
  onNativeDevicesUpdated: (callback: (devices: NativeDeviceSummary[]) => void) => {
    const listener = (_event: IpcRendererEvent, devices: NativeDeviceSummary[]) => callback(devices);

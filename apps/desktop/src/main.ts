...
function setupFileProtocol() {
  ...
  protocol.registerFileProtocol("app", (request, callback) => {
    ...
    if (url.includes("index.html/assets/") || url.includes("index.html/")) {
      ...
      console.log(
        `[app://] Rewriting malformed URL: ${request.url} -> app://${url}`,
      );
    }
    ...
    try {
      if (fs.existsSync(filePath)) {
        ...
        console.error(
          `[app://] File not found: ${filePath} (requested: ${request.url}, cleaned: ${url})`,
        );
    ...
    if (app.isPackaged && fileUrl.includes("app.asar")) {
      ...
      if (asarMatch) {
        ...
        try {
          ...
          console.error(
            `[file://] Error accessing asar file: ${filePath}`,
            error,
          );
        }
      }
  ...
  if (!isDev && mainWindow) {
    ...
    webSession.webRequest.onBeforeRequest(
      ...
      (details, callback) => {
        ...
        if (
          details.url.includes("index.html/assets/") ||
          details.url.includes("index.html/")
        ) {
          const correctedUrl = details.url.replace(/index\.html\//g, "");
          console.log(
            `[webRequest] Rewriting URL: ${details.url} -> ${correctedUrl}`,
          );

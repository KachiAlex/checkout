  ...
  const { logout: i, accessToken: m } = e(),
    ...
    j = t.useCallback(async () => {
      if (m) {
        h(!0);
        try {
          const e = await s.get(`${a}/api/v1/suppliers`, {
            headers: { Authorization: `Bearer ${m}` },
          });
          x(e.data || []);
        } catch (e) {
          401 !== e.response?.status && r.error("Failed to load suppliers");
        } finally {
          h(!1);
        }
      }
              ...
              l.jsxs("form", {
                onSubmit: async (e) => {
                  if ((e.preventDefault(), m))
                    try {
                      (f
                        ? (await s.patch(`${a}/api/v1/suppliers/${f.id}`, g, {
                            headers: { Authorization: `Bearer ${m}` },
                          }),
                          r.success("Supplier updated successfully"))
                        : (await s.post(`${a}/api/v1/suppliers`, g, {
                            headers: { Authorization: `Bearer ${m}` },
                          }),
                          r.success("Supplier created successfully")),
                        b(!1),
                        y(null),
                        N({
                          name: "",
                      ...
                      r.error(
                        t.response?.data?.message || "Failed to save supplier",
                      );
                    }

  ...
  return (
    s.useEffect(() => {
      const e = async () => {
        if (!x || !u)
          return (
            d("failed"),
            void r.error("Invalid payment callback parameters")
          );
        try {
          const t = await n.get(
            `${i}/api/v1/platform/subscriptions/${x}/payment/status/${u}`,
          );
          "completed" === t.data.status
            ? (d("success"),
              r.success("Payment successful! Your subscription is now active."),
              setTimeout(() => {
                const e = t.data.tenantSlug || x;
                c(`/${e}/login`);
              }, 3e3))
            : "failed" === t.data.status
              ? (d("failed"), r.error("Payment failed. Please try again."))
              : setTimeout(e, 2e3);
        } catch (t) {
          (d("failed"),
            r.error(
    ...
    a.jsx("div", {
      ...
      children: a.jsxs("div", {
        ...
        children: [
          a.jsx(l, { className: "mx-auto mb-6" }),
          "loading" === o &&
            a.jsxs(a.Fragment, {
              children: [
                a.jsx("div", {

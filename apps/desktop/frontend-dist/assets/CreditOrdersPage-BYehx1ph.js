...
import { f as l } from "./format-BJFo2Mkz.js";
function o() {
      ...
      if (m) {
        ...
        try {
          a.defaults.headers.common.Authorization ||
            (a.defaults.headers.common.Authorization = `Bearer ${m}`);
          const e =
            (
              await a.get(`${r}/api/v1/orders/credit`, {
          ...
          let t = new Map();
          try {
            const e = await a.get(`${r}/api/v1/users`, {
              headers: { Authorization: `Bearer ${m}` },
            });
            (e.data || []).forEach((e) => {
              t.set(e.id, e);
          ...
          const s = [...new Set(e.map((e) => e.customerId).filter(Boolean))],
            n = new Map();
          s.length > 0 &&
            (await Promise.all(
              s.map(async (e) => {
                try {
                  const t = await a.get(`${r}/api/v1/customers/${e}`, {
                    headers: { Authorization: `Bearer ${m}` },
                  });
                  t.data && n.set(e, t.data);
                } catch (t) {}
              }),
            ));
          const d = await Promise.all(
            e.map(async (e) => {
              const s = (e.customerId && n.get(e.customerId)) || null;
              e.customerId;
              const d = await Promise.all(
                  e.items.map(async (e) => {
                    try {
                      const t = await a.get(
                        `${r}/api/v1/products/${e.productId}`,
                        { headers: { Authorization: `Bearer ${m}` } },
                      );
                      return { ...e, product: t.data };
                    } catch (i) {
                      return e;
                    }
                  }),
                ),
                l = t.get(e.createdBy) || null;
              return {
                ...e,
                customer: s,
                items: d,
          ...
          401 === (null == (e = i.response) ? void 0 : e.status)
            ? n.error("Authentication failed. Please log in again.")
            : n.error(
                (null == (s = null == (t = i.response) ? void 0 : t.data)
                  ? void 0
                  : s.message) || "Failed to load credit orders",
              );
        } finally {
          p(!1);
        }
                          ...
                          onClick: () => {
                            ((async (e) => {
                              ...
                              if (
                                ...
                                try {
                                  (await a.post(
                                    `${r}/api/v1/orders/${e}/credit/mark-paid`,
                                    {},
                                    {
                                      headers: { Authorization: `Bearer ${m}` },
                                    },
                                  ),
                                    n.success("Credit order marked as paid"),
                                    N());
                                } catch (i) {
                                  n.error(
                                    (null ==
                          ...
                          onClick: () => {
                            ((async (e) => {
                              ...
                              if (
                                ...
                                try {
                                  (await a.post(
                                    `${r}/api/v1/orders/${e}/credit/mark-returned`,
                                    {},
                                    {
                                      headers: { Authorization: `Bearer ${m}` },
                                    },

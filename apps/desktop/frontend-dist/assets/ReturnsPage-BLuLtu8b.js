...
function o() {
  const { accessToken: o } = e(),
    ...
    k = t.useCallback(async () => {
      if (o) {
        ...
        try {
          const e = h ? { search: h } : {},
            t = await r.get(`${s}/api/v1/returns`, {
              headers: { Authorization: `Bearer ${o}` },
              params: e,
            });
          i(t.data || []);
        } catch (e) {
          401 !== e.response?.status && a.error("Failed to load returns");
        } finally {
          m(!1);
        }
      }
  ...
  const C = async (e) => {
      if (o && e)
        try {
          const t = await r.get(`${s}/api/v1/orders`, {
            headers: { Authorization: `Bearer ${o}` },
          ...
          if (t.data && t.data.length > 0) {
            ...
            const a = await Promise.all(
              e.items.map(async (e) => {
                try {
                  const t = await r.get(`${s}/api/v1/products/${e.productId}`, {
                    headers: { Authorization: `Bearer ${o}` },
                  });
                  return {
                    orderItemId: e.id,
            ...
            v(a);
          } else a.error("Order not found");
        } catch (t) {
          a.error(t.response?.data?.message || "Failed to search order");
        }
    },
    I = (e, t) => {
      v((r) =>
        r.map((r) => {
          if (r.orderItemId === e) {
            const s = r.quantity,
              a = Math.min(Math.max(0, t), s),
              n = Math.round(
                (a / r.quantity) *
                  (b?.items.find((t) => t.id === e)?.priceCents || 0) *
                  a,
              );
            return { ...r, returnQuantity: a, refundAmountCents: n };
          }
          return r;
        }),
      );
                        ...
                        n.jsxs("div", {
                          ...
                          children: [
                            ...
                            n.jsx("button", {
                              onClick: async () => {
                                if (!o || !b) return;
                                const e = f.filter((e) => e.returnQuantity > 0);
                                if (0 !== e.length)
                                  try {
                                    (await r.post(
                                      `${s}/api/v1/returns`,
                                      ...
                                      a.success("Return request created"),
                                      x(!1),
                                      y(null),
                                      v([]),
                                      g("CUSTOMER_REQUEST"),
                                ...
                                else a.error("Please select items to return");
                              },
                              disabled:
                                0 ===

const __vite__mapDeps = (
  ...
  d = m.f ||
    (m.f = [
      ...
      "assets/index-CcrCVdax.js",
      "assets/index-C-MD2CNG.css",
    ]),
) => i.map((i) => d[i]);
import { a as e, e as t, A as n, _ as r } from "./index-CcrCVdax.js";
const i = () => {
...
const s = new (class {
  ...
  registeredPrinters = new Map();
  messageHandlers = new Map();
  async getReceipt(r) {
    const i = e.getState().accessToken;
    if (!i) throw new Error("Not authenticated");
    return (
      await t.get(`${n}/api/v1/receipts/${r}`, {
        headers: { Authorization: `Bearer ${i}` },
      })
    ).data.receipt;
  }
  async sendEmailReceipt(r, i) {
    const s = e.getState().accessToken;
    if (!s) throw new Error("Not authenticated");
    try {
      return (
        await t.post(
          `${n}/api/v1/receipts/${r}/email`,
          { email: i },
  ...
  handleMessage(e) {
    if (
      !e.error &&
      (e.success && e.message?.includes("registered"),
  ...
  async printReceipt(r, i) {
    try {
      const s = e.getState().accessToken;
      if (!s) throw new Error("Not authenticated");
      const o = await t.get(`${n}/api/v1/receipts/${r}/print`, {
          headers: { Authorization: `Bearer ${s}` },
        }),
        { escpos: a } = o.data;
      if (
        ...
        throw new Error("Not connected to print proxy");
      const c = i || "default-printer";
      return new Promise((e) => {
        (this.messageHandlers.set(`print-${r}`, (t) => {
          (t.success ? e(!0) : e(!1),
            this.messageHandlers.delete(`print-${r}`));
        }),
          this.ws.send(
            JSON.stringify({
              type: "print",
          ...
          setTimeout(() => {
            this.messageHandlers.has(`print-${r}`) &&
              (this.messageHandlers.delete(`print-${r}`), e(!1));
          }, 1e4));
      });
    } catch (s) {
      return !1;
  ...
  async printReceiptToSerial(i, s) {
    try {
      const o = e.getState().accessToken;
      if (!o) throw new Error("Not authenticated");
      const a = await t.get(`${n}/api/v1/receipts/${i}/print`, {
          headers: { Authorization: `Bearer ${o}` },
        }),
        { escpos: c } = a.data,
        d = new TextEncoder().encode(c),
        { writeToSerialPort: h } = await r(
          async () => {
            const { writeToSerialPort: e } = await import(
              "./printerDeviceService-lpuUQ9W6.js"
            );
            return { writeToSerialPort: e };
          },
          __vite__mapDeps([0, 1, 2]),
        );
      return (await h(s, d), !0);
    } catch (o) {
      return !1;
    }
  }
  async printReceiptToBluetooth(i, s) {
    try {
      const o = e.getState().accessToken;
      if (!o) throw new Error("Not authenticated");
      const a = await t.get(`${n}/api/v1/receipts/${i}/print`, {
        ...
        { escpos: c } = a.data,
        d = new TextEncoder().encode(c),
        { writeToBluetoothPrinter: h } = await r(
          async () => {
  ...
  async isAvailable() {
    try {
      return (
        !!i() && (await this.connect(), this.ws?.readyState === WebSocket.OPEN)
      );
    } catch (e) {
      return !1;
    }

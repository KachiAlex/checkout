...
export class LocationsController {
  ...
  async create(@Req() req: AuthenticatedRequest, @Body() createDto: CreateLocationInput) {
    const location = await this.locationsRepository.create({
      ...createDto,
    });
    // Update location with tenantId
    await this.locationsRepository.update(location.id, { tenantId: req.user.tenantId });

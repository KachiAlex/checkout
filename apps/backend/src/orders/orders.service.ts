...
export class OrdersService {
  ...
  async create(
    ...
    if (isCreditOrder && !createOrderDto.customerId) {
      throw new BadRequestException(
        'Customer is required for credit orders. Please select or create a customer.',
      );
    }
    ...
    console.log(
      `✅ Order created and saved: ${order.id} (${order.orderNumber}) for tenant ${tenantId}, status: ${order.status}, locationId: ${locationId}, customerId: ${order.customerId || 'none'}, isCreditOrder: ${isCreditOrder}, createdAt: ${order.createdAt.toISOString()}`,
    );

    // Award loyalty points if order is completed and has a customer
    if (
      !createOrderDto.isHeld &&
  ...
  private async validateAndDecrementInventory(
    dto: CreateOrderDto,
    userId: string,
  ...
  private async generateOrderNumber(locationId: string): Promise<string> {
    ...
    const locationPrefix =
      locationId.length >= 4 ? locationId.substring(0, 4).toUpperCase() : 'DEFT';
  ...
  async findAll(
    ...
    const filteredLocationId = locationId;
  ...
  async completeHeldOrder(id: string, tenantId: string): Promise<OrderRecord> {
    ...
    await this.validateAndDecrementInventory(
      ...
      order.createdBy,
      order.isCreditOrder ?? false,
    ); // Pass userId from order
  ...
  async findCreditOrders(locationId?: string, tenantId?: string): Promise<OrderRecord[]> {
    try {
      ...
      console.log(
        `📋 Found ${orders.length} credit orders for tenant ${tenantId}, location ${locationId || 'all'}`,
      );
      // Log customerIds from orders
      orders.forEach((order) => {
        console.log(
          `  Order ${order.orderNumber} (${order.id}): customerId=${order.customerId || 'none'}`,
        );
      });

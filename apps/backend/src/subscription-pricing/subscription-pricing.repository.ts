...
export class SubscriptionPricingRepository {
  ...
  async update(updates: Partial<SubscriptionPricingEntity>): Promise<SubscriptionPricingEntity> {
    ...
    if (this.isPostgresEnabled()) {
      await this.prismaService.prisma.subscriptionPricing.upsert({
        where: { id: DEFAULT_DOC_ID },
        update: {
          free: updated.free as unknown as Prisma.JsonValue,
          starter: updated.starter as unknown as Prisma.JsonValue,
          professional: updated.professional as unknown as Prisma.JsonValue,
          enterprise: updated.enterprise as unknown as Prisma.JsonValue,
          lifetime: updated.lifetime as unknown as Prisma.JsonValue,
  ...
  private mapPrismaRowToEntity(row: PrismaSubscriptionPricing): SubscriptionPricingEntity {
    return {
      id: row.id,
      free: row.free as unknown as SubscriptionPricingEntity['free'],
      starter: row.starter as unknown as SubscriptionPricingEntity['starter'],
      professional: row.professional as unknown as SubscriptionPricingEntity['professional'],
      enterprise: row.enterprise as unknown as SubscriptionPricingEntity['enterprise'],
      lifetime: row.lifetime as unknown as SubscriptionPricingEntity['lifetime'],

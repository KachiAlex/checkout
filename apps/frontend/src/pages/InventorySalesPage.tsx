...
export function InventorySalesPage() {
  ...
  const loadInventoryStock = useCallback(async () => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        toast.error(
          error.response?.data?.message || "Failed to load inventory stock",
        );
      }
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    if (user?.locationId && accessToken) {
      ...
      console.warn(
        "InventorySalesPage: Missing user location or access token",
        {
  ...
  const handleAdjustInventory = async () => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/api/v1/inventory/adjust`, {
          ...
          <div className="flex items-start gap-3 sm:gap-4 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="theme-text-secondary text-xs sm:text-sm">
                Store: {user?.locationId || "store-001"}
              </p>
            </div>
          </div>
                  ...
                  <tbody className="divide-y divide-white/10">
                    {inventoryStock.length === 0 ? (
                      <tr>
                        <td
                          colSpan={8}
                          className="px-6 py-8 text-center theme-text-secondary"
                        >
                          No inventory items found. Products will appear here
                    ...
                    {inventoryTransactions.length === 0 ? (
                      ...
                      inventoryTransactions.map((transaction) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`font-semibold ${transaction.delta < 0 ? "text-red-600" : "text-green-600"}`}
                            >
                              {transaction.delta > 0 ? "+" : ""}
                              {transaction.delta}
                            </span>

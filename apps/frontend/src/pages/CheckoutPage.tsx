...
export function CheckoutPage() {
  ...
  const handlePayment = async (method: 'card' | 'cash' | 'qr') => {
    ...
    try {
      ...
      // Create order
      const orderUuid = `order_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      ...
      if (payment.status === 'completed') {
        toast.success(`Payment successful! Order: ${order.orderNumber}`);
        
        // Generate receipt
        try {
          const receiptResponse = await axios.get(
            `${API_URL}/api/v1/receipts/${order.id}`,
          );

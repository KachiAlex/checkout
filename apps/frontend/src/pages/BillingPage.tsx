...
export function BillingPage() {
    ...
    try {
      ...
      const pricingPayload: Partial<SubscriptionPricing> = {
        ...
        starter: {
          ...
          locations:
            pricingForm.starter?.locations ??
            pricingConfig.starter?.locations ??
            1,
          users:
            pricingForm.starter?.users ?? pricingConfig.starter?.users ?? 10,
        },
        professional: {
          ...pricingConfig.professional!,
          priceCents: Math.round((pricingFormDollars.professional ?? 0) * 100),
          locations:
            pricingForm.professional?.locations ??
            pricingConfig.professional?.locations ??
            5,
          users:
            pricingForm.professional?.users ??
            pricingConfig.professional?.users ??
            15,
        },
        enterprise: {
          ...
          locations:
            pricingForm.enterprise?.locations ??
            pricingConfig.enterprise?.locations ??
            0,
          users:
            pricingForm.enterprise?.users ??
            pricingConfig.enterprise?.users ??
            0,
        },
        lifetime: {
          ...
          locations:
            pricingForm.lifetime?.locations ??
            pricingConfig.lifetime?.locations ??
            0,
          users:
            pricingForm.lifetime?.users ?? pricingConfig.lifetime?.users ?? 0,
        },
      };
  ...
  const handleEditPromo = (promo: PromoDiscount) => {
    ...
    // Convert cents to dollars for display
    setPromoFormDollars({
      minPurchase: promo.minPurchaseCents
        ? promo.minPurchaseCents / 100
        : undefined,
      maxDiscount: promo.maxDiscountCents
        ? promo.maxDiscountCents / 100
        : undefined,
      discountValue:
        promo.discountType === "fixed" ? promo.discountValue / 100 : undefined,
    });
    setShowPromoModal(true);
  };
  ...
  const handleSavePromo = async () => {
    ...
    if (
      ...
      (promoForm.discountValue < 0 || promoForm.discountValue > 100)
    ) {
    ...
    try {
      ...
      const promoPayload: CreatePromoDiscountPayload = {
        ...
        discountValue:
          ...
          promoFormDollars.discountValue !== undefined
            ? Math.round(promoFormDollars.discountValue * 100)
            : promoForm.discountValue,
        minPurchaseCents: promoFormDollars.minPurchase
          ? Math.round(promoFormDollars.minPurchase * 100)
          : undefined,
        maxDiscountCents: promoFormDollars.maxDiscount
          ? Math.round(promoFormDollars.maxDiscount * 100)
          : undefined,
      };
      ...
      toast.error(
        error?.response?.data?.error || "Failed to save promo discount",
      );
    }
  };
  ...
  const handleDeletePromo = async (id: string) => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.error || "Failed to delete promo discount",
      );
    }
  };
          ...
          <div className="flex items-start gap-3 sm:gap-4 min-w-0">
            ...
            <div>
              ...
              <p className="theme-text-secondary mt-2 text-sm md:max-w-xl">
                Configure subscription pricing tiers and manage promotional
                discount codes.
              </p>
            </div>
          </div>
                ...
                <div className="space-y-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min="1"
                      value={
                        pricingForm.starter?.users ??
                        pricingConfig.starter?.users ??
                        10
                      }
                      onChange={(e) =>
                        setPricingForm((prev) => ({
                ...
                <div className="space-y-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      value={
                        pricingForm.lifetime?.users ??
                        pricingConfig.lifetime?.users ??
                        0
                      }
                      onChange={(e) =>
                        setPricingForm((prev) => ({
          ...
          {loadingPromos ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-white/10 text-sm">
                <thead>
                  <tr>
                    <th className="theme-text-secondary px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">
                      Code
                    </th>
                    <th className="theme-text-secondary px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">
                      Name
                    ...
                    <th className="theme-text-secondary px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {promoDiscounts.map((promo) => (
                    <tr key={promo.id} className="hover:bg-white/5">
                      <td className="theme-text-primary px-4 py-3 font-mono font-semibold">
                        {promo.code}
                      </td>
                      ...
                      <td className="theme-text-secondary px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {promo.applicablePlans.map((plan) => (
                            <span
                              key={plan}
                              className="theme-chip rounded-full border px-2 py-0.5 text-xs capitalize"
                            >
                              {plan}
                            </span>
                          ))}

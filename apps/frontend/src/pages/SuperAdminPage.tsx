...
export function SuperAdminPage() {
  ...
  const [subscriptionOriginal, setSubscriptionOriginal] = useState<
    typeof subscriptionForm | null
  >(null);
  ...
  const [pricingConfig, setPricingConfig] =
    useState<SubscriptionPricing | null>(null);
    ...
    const initial = {
      ...
      billingCycleEnd:
        tenant.plan === "lifetime"
          ...
          : tenant.plan === "annual"
            ? (() => {
                const startDate = tenant.billingCycleStart
                  ? new Date(tenant.billingCycleStart)
                  : null;
                return startDate
                  ? addOneYear(startDate).toISOString().slice(0, 10)
                  : "";
              })()
  ...
  const updateSubscriptionField = (
    field: keyof typeof subscriptionForm,
    value: string,
  ) => {
    setSubscriptionForm((prev) => {
      ...
      if (field === "plan") {
        if (value === "lifetime") {
          ...
          next.billingCycleEnd = addOneYear(baseDate)
            .toISOString()
            .slice(0, 10);
    ...
    try {
      ...
      if (activateImmediately) {
        try {
          ...
          toast.success(`${tenantRecord.name} activated immediately`, {
            duration: 6000,
          });
        } catch (activateError: any) {
          toast.error(
            activateError?.response?.data?.message ||
              `${result.tenant.name} created, but activation failed`,
          );
        }
      }
  ...
  const handleSubscriptionSubmit = async (event: React.FormEvent) => {
    ...
    if (
      subscriptionForm.billingCycleStart !==
      subscriptionOriginal.billingCycleStart
    ) {
    ...
    if (subscriptionForm.plan === "lifetime") {
      if (
        subscriptionOriginal.plan !== "lifetime" ||
        subscriptionOriginal.billingCycleEnd !== ""
      ) {
        updates.billingCycleEnd = null;
        hasChanges = true;
      }
    } else if (
      subscriptionForm.billingCycleEnd !== subscriptionOriginal.billingCycleEnd
    ) {
    ...
    try {
      const updatedTenant = await updateTenantSubscription(
        subscriptionTenant.id,
        updates,
      );
      setTenants((prev) =>
        prev.map((tenant) =>
  ...
  const handleResetPin = async (tenant: TenantSummary) => {
    ...
    try {
      const response = await resetTenantAdminPin(
        tenant.id,
        tenant.contactEmail,
      );
      toast.success(
      ...
      toast.error(
        error?.response?.data?.message || "Unable to reset admin PIN",
      );
    } finally {
      setActionKey(null);
    }
  ...
  const handleSuspend = async (tenant: TenantSummary) => {
    const reason =
      window.prompt(
        `Provide a suspension reason for ${tenant.name} (optional):`,
      ) || "";
    setActionKey(`${tenant.id}:suspend`);
    try {
      const updatedTenant = await suspendTenant(tenant.id, {
  ...
  const handleActivate = async (tenant: TenantSummary) => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Unable to activate tenant",
      );
    } finally {
      setActionKey(null);
    }
  ...
  const handleDelete = async (tenant: TenantSummary) => {
    ...
    try {
      ...
      setLastProvisioned((prev) =>
        prev?.tenant.id === tenant.id ? null : prev,
      );
  ...
  const handleSavePricing = async () => {
    ...
    try {
      ...
      const updated = await updateSubscriptionPricing(
        pricingPayload,
        accessToken,
      );
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-4 sm:gap-6 lg:gap-8 px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-10">
            ...
            <div className="mt-3 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-4 py-3">
              <p className="text-sm">
                ...
                <span className="font-mono text-lg text-emerald-100">
                  {lastProvisioned.admin.temporaryPin}
                </span>
              </p>
              <p className="mt-1 text-xs text-emerald-200/80">
                The tenant admin can use this PIN to log in and should change it
                after the first login. The tenant is currently{" "}
                <span className="font-semibold">
                  {lastProvisioned.tenant.status}
                </span>
                .
              </p>
            </div>
        ...
        <header className="flex flex-col gap-4 sm:gap-6 md:flex-row md:items-center md:justify-between">
          <div className="flex items-start gap-3 sm:gap-4 min-w-0">
            ...
            />
            <div>
              <p className="theme-text-secondary text-xs uppercase tracking-[0.35em]">
                Platform control
              </p>
              ...
              <p className="theme-text-secondary mt-2 text-sm md:max-w-xl">
                Provision new tenant companies, assign plans, and monitor
                rollout progress. This workspace is isolated from the
                tenant-facing POS.
              </p>
            </div>
          </div>
        ...
        <section className="grid gap-4 md:grid-cols-4">
          <div className="theme-card rounded-3xl border px-5 py-6 backdrop-blur-xl">
            <p className="theme-text-secondary text-xs uppercase tracking-[0.3em]">
              Total tenants
            </p>
            <h2 className="theme-text-primary mt-4 text-3xl font-semibold">
              {stats.total}
            </h2>
          </div>
          <div className="theme-card rounded-3xl border px-5 py-6 backdrop-blur-xl">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between mb-6">
            <div>
              ...
              <p className="theme-text-secondary mt-1 text-xs">
                Configure monthly prices for each subscription tier. Prices are
                in dollars.
              </p>
            </div>
          ...
          {loadingPricing ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4">
                <h3 className="theme-text-primary text-sm font-semibold text-emerald-400 mb-2">
                  Free (14-day trial)
                </h3>
                ...
                <div className="space-y-2">
                  <div>
                    <label className="theme-text-secondary text-xs mb-1 block">
                      Price ($)
                    </label>
                    <input
                      type="number"
                      value={(
                        (pricingConfig?.free?.priceCents ?? 0) / 100
                      ).toFixed(2)}
                      disabled
              ...
              <div className="rounded-2xl border border-sky-500/30 bg-sky-500/10 p-4">
                <h3 className="theme-text-primary text-sm font-semibold text-sky-400 mb-2">
                  Starter
                </h3>
                <p className="theme-text-secondary text-xs mb-3">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setPricingForm((prev) => ({
                          ...
                          starter: {
                            ...prev.starter!,
                            locations: parseInt(e.target.value) || 1,
                          },
                        }))
                      }
                      className="theme-surface w-full rounded-xl border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-400"
              ...
              <div className="rounded-2xl border border-purple-500/30 bg-purple-500/10 p-4">
                <h3 className="theme-text-primary text-sm font-semibold text-purple-400 mb-2">
                  Professional
                </h3>
                <p className="theme-text-secondary text-xs mb-3">
                  Monthly subscription
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setPricingForm((prev) => ({
                          ...
                          professional: {
                            ...prev.professional!,
                            locations: parseInt(e.target.value) || 5,
                          },
                        }))
                      }
                      className="theme-surface w-full rounded-xl border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-purple-400"
              ...
              <div className="rounded-2xl border border-indigo-500/30 bg-indigo-500/10 p-4">
                <h3 className="theme-text-primary text-sm font-semibold text-indigo-400 mb-2">
                  Enterprise
                </h3>
                <p className="theme-text-secondary text-xs mb-3">
                  Custom pricing
        ...
        <section className="theme-card rounded-3xl border p-6 backdrop-blur-xl">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="theme-text-secondary mt-1 text-xs">
                Slugs are unique, lowercase identifiers. They become the tenant
                URL:
            ...
            <div className="flex flex-col gap-2">
              <div className="rounded-xl border border-sky-500/30 bg-sky-500/10 px-4 py-3">
                <p className="text-sm text-sky-200">
                  <strong>Auto-generated PIN:</strong> A temporary 6-digit PIN
                  will be automatically generated for the tenant admin. This PIN
                  will be displayed after tenant creation.
                </p>
              </div>
            </div>
            <div className="flex flex-col gap-2">
                ...
                <button
                  ...
                  onClick={() =>
                    setForm((prev) => ({
                      ...
                      billingCycleStart: "",
                      billingCycleEnd:
                        prev.plan === "annual" ? "" : prev.billingCycleEnd,
                    }))
                  }
                >
                  Schedule start
              ...
              {form.plan === "annual" &&
                ...
                !form.billingCycleStart && (
                  <p className="theme-text-secondary text-[11px]">
                    Choose a start date to auto-calculate the annual billing
                    window.
                  </p>
                )}
            </div>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="theme-text-secondary text-xs">
                Monitor licensing posture and quick-launch into tenant
                environments.
              </p>
            </div>
            <input
                ...
                {loading ? (
                  ...
                  filteredTenants.map((tenant) => (
                    <tr key={tenant.id} className="hover:bg-white/5 transition">
                      ...
                      <td className="px-4 py-4">
                        <code className="rounded bg-white/10 px-2 py-1 text-xs lowercase">
                          {tenant.slug}
                        </code>
                      </td>
                      <td className="px-4 py-4 capitalize theme-text-secondary">
                        {tenant.plan}
                      </td>
                      <td className="px-4 py-4 text-xs uppercase">
                        <StatusBadge status={tenant.status} />
                        {tenant.status === "suspended" &&
                          Boolean(tenant.metadata?.suspensionReason) && (
                        ...
                        <div className="flex w-full flex-wrap items-center justify-end gap-2 sm:gap-3">
                          ...
                          {tenant.status !== "active" &&
                            tenant.status !== "suspended" && (
                              ...
                              >
                                {isActionBusy(`${tenant.id}:activate`)
                                  ? "Activating…"
                                  : "Activate now"}
                              </button>
                            )}
                          ...
                          {tenant.status === "suspended" ? (
                            ...
                            >
                              {isActionBusy(`${tenant.id}:suspend`)
                                ? "Suspending…"
                                : "Suspend"}
                            </button>
                          )}
              ...
              >
                <div className="flex flex-col gap-2">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateSubscriptionField("plan", event.target.value)
                    }
                    className="theme-surface rounded-2xl border px-4 py-3 outline-none focus:ring-2 focus:ring-sky-400"
                  >
                    {PLAN_OPTIONS.map((plan) => (
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateSubscriptionField("seatLimit", event.target.value)
                    }
                    className="theme-surface rounded-2xl border px-4 py-3 outline-none focus:ring-2 focus:ring-sky-400"
                  />
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <input
                    ...
                    className="theme-surface rounded-2xl border px-4 py-3 outline-none focus:ring-2 focus:ring-sky-400 disabled:opacity-60"
                    disabled={
                      subscriptionForm.plan === "lifetime" ||
        ...
        {passwordModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="theme-card w-full max-w-md rounded-3xl border p-6 backdrop-blur-xl">
              ...
              <p className="theme-text-secondary mt-1 text-xs">
                Update your superadmin password. Use a strong password with at
                least 6 characters.
              </p>
